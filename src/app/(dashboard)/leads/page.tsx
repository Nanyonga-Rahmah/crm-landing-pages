...
const LeadsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      ...
      try {
        ...
        const validStatuses = ['LEAD', 'QUALIFIED'];
        const fetchedLeads = data.data.leads.filter((lead: ILeads) =>
          validStatuses.includes(lead.leadStatus || '')
        );
  ...
  useEffect(() => {
    const filterAndSearchLeads = () => {
      const filteredLeads = allLeads.filter((lead: ILeads) => {
        const categoryMatch = lead.leadStatus
          ? lead.leadStatus.toLowerCase().includes(filterCategory.toLowerCase())
          : false;
        const searchMatch =
          searchQuery === ''
            ? true
  ...
  }, [filterCategory, searchQuery, allLeads]);

  const breadcrumbItems = [{ href: '/leads', label: 'Leads' }];
  ...
  return (
    <>
      <div className="overflow-hidden">
        <DashboardPageHeader pageTitle="Leads" />
        <div className="pt-6 pl-8 text-xl">
          <BreadCrumb items={breadcrumbItems} />
        </div>
        <div className="m-8 shadow-sm bg-white grid grid-rows-1 p-4 rounded-md border border-gray-300 overflow-auto">
          <div className="header flex flex-col gap-2 p-4 mb-3 md:flex-row justify-between items-center">
            <div className="flex gap-2">
              <h3 className="text-xl mt-2">Leads</h3>
